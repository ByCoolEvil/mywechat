...
class MenuController extends Controller
{
    ...
    {
        $info = DB::connection('mysql_wechat')->table('menu')->orderBy('name1','asc','name2','asc')->get();
    ...
    public function create_menu(Request $request)
    {
        ...
        $result = DB::connection('mysql_wechat')->table('menu')->insert([
            'name1' => $req['name1'],
            'name2' => $req['name2'],
            'type' => $req['type'],
...
//        将循环的数据转换成数组
        $data = [];
        $menu_list = DB::connection('mysql_wechat')->table('menu')->select(['name1'])->groupBy('name1')->get();
        foreach($menu_list as $vv){
            $menu_info = DB::connection('mysql_wechat')->table('menu')->where(['name1'=>$vv->name1])->get();
            $menu = [];
    ...
    {
        $id = $request->all()['id'];
        $del_result = DB::connection('mysql_wechat')->table('menu')->where(['id'=>$id])->delete();
        if(!$del_result){
            dd('删除失败');
        }
//        根据表数据翻译成菜单结构
